...
export async function GET(req: NextRequest) {
  try {
    ...
    requireRole(session, PERMISSIONS.groupe.read);
    ...
    const comparatif = await getGroupeComparatif(session.organizationId, from, to);

    return NextResponse.json(comparatif);
  } catch (error) {
