...
interface LeaveRequest {
  id: string;
  type: "CONGE_PAYE" | "CONGE_MALADIE" | "CONGE_MATERNITE" | "CONGE_SANS_SOLDE" | "AUTRE";
  dateDebut: string;
  dateFin: string;
...
const LEAVE_TYPE_LABEL: Record<LeaveRequest["type"], string> = {
...
function leaveStatusVariant(status: LeaveRequest["status"]): "success" | "warning" | "destructive" | "secondary" {
...
const CAN_MANAGE_LEAVES = ["ADMIN", "RH"];
const emptyLeaveForm = { type: "CONGE_PAYE" as LeaveRequest["type"], dateDebut: "", dateFin: "", motif: "" };
...
export default function RhPage() {
  ...
  const canManageLeaves = user && CAN_MANAGE_LEAVES.includes(user.role);

  const { data: usersData } = useQuery({ queryKey: ["users"], queryFn: () => api.get<{ users: User[] }>("/api/users"), enabled: !!canManageLeaves });
  const { data: shiftsData } = useQuery({ queryKey: ["rh", "gardes"], queryFn: () => api.get<{ shifts: Shift[] }>("/api/rh/gardes"), enabled: !!canManageLeaves });
  ...
  const { data: leavesData } = useQuery({ queryKey: ["rh", "conges"], queryFn: () => api.get<{ leaveRequests: LeaveRequest[] }>("/api/rh/conges") });

  const createLeave = useMutation({
    mutationFn: () => api.post("/api/rh/conges", leaveForm),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["rh", "conges"] });
  ...
  const reviewLeave = useMutation({
    mutationFn: ({ id, status }: { id: string; status: "APPROUVE" | "REJETE" }) => api.patch(`/api/rh/conges/${id}`, { status }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["rh", "conges"] }),
    onError: reportError,
  });

  const cancelLeave = useMutation({
    mutationFn: (id: string) => api.patch(`/api/rh/conges/${id}`, { status: "ANNULE" }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["rh", "conges"] }),
    onError: reportError,
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold">RH & congés</h1>
        <p className="text-sm text-muted-foreground">
          {canManageLeaves ? "Planning des gardes, paie et gestion des congés" : "Vos demandes de congés"}
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0">
          <CardTitle className="text-base">{canManageLeaves ? "Congés de l'équipe" : "Mes congés"}</CardTitle>
          <Button size="sm" onClick={() => setShowLeaveForm((v) => !v)}>{showLeaveForm ? "Annuler" : "Demander un congé"}</Button>
        </CardHeader>
        <CardContent className="space-y-4">
          {showLeaveForm && (
            <div className="grid grid-cols-1 gap-3 rounded-md border p-4 sm:grid-cols-2">
              <Select value={leaveForm.type} onChange={(e) => setLeaveForm({ ...leaveForm, type: e.target.value as LeaveRequest["type"] })}>
              ...
              <div className="space-y-1.5">
                ...
                <Input type="date" value={leaveForm.dateDebut} onChange={(e) => setLeaveForm({ ...leaveForm, dateDebut: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label>Date de fin</Label>
                <Input type="date" value={leaveForm.dateFin} onChange={(e) => setLeaveForm({ ...leaveForm, dateFin: e.target.value })} />
              </div>
              <Input placeholder="Motif (optionnel)" className="sm:col-span-2" value={leaveForm.motif} onChange={(e) => setLeaveForm({ ...leaveForm, motif: e.target.value })} />
              <Button disabled={!leaveForm.dateDebut || !leaveForm.dateFin || createLeave.isPending} onClick={() => createLeave.mutate()}>
                Envoyer la demande
              </Button>
            </div>
          )}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                {canManageLeaves && <TableHead>Employé</TableHead>}
            ...
            <TableBody>
              {leavesData?.leaveRequests.map((leave) => (
                <TableRow key={leave.id}>
                  {canManageLeaves && <TableCell>{leave.user.firstName} {leave.user.lastName}</TableCell>}
                  <TableCell>{LEAVE_TYPE_LABEL[leave.type]}</TableCell>
                  ...
                  <TableCell className="whitespace-nowrap">
                    {leave.status === "EN_ATTENTE" && canManageLeaves && (
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => reviewLeave.mutate({ id: leave.id, status: "APPROUVE" })}>Approuver</Button>
                        <Button size="sm" variant="outline" onClick={() => reviewLeave.mutate({ id: leave.id, status: "REJETE" })}>Rejeter</Button>
                      </div>
                    )}
                    {leave.status === "EN_ATTENTE" && !canManageLeaves && (
                      <Button size="sm" variant="outline" onClick={() => cancelLeave.mutate(leave.id)}>Annuler</Button>
                    )}
                  </TableCell>
                </TableRow>
              ))}
              {leavesData?.leaveRequests.length === 0 && (
                <TableRow>
                  <TableCell colSpan={canManageLeaves ? 5 : 4} className="text-center text-muted-foreground">Aucune demande</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
